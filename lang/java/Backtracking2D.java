...
public class Backtracking2D {
	...
	public static boolean[] sudGetFixed(final byte[] sud) {
		final boolean[] z = new boolean[81];
		for (int r = 0; r < 9; r++) {
			for (int c = 0; c < 9; c++) {
				z[9*r+c] = sud[9*r+c] > 0;
			}
	...
	public static boolean sudIsValid(final byte[] sud) {
		...
		for (int r = 0; r < 9; r++) {
			for (int c = 0; c < 9; c++) {
				if (sud[9*r+c] > 0 && !sudCellIsValid(sud, r, c)) {
					return false;
				}
			}
		}
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(path)))
		{
			...
			while ((line = reader.readLine()) != null)
			{
				...
				for (int c = 0; c < line.length(); c++) {
					sud[9*sudRow+c] = (byte)(line.charAt(c) - '0');
				}
				sudRow++;
			}
		} catch (Exception e) {
	...
	public static void sudSolve(final byte[] sud) {
		...
		final byte[] sudCP = Arrays.copyOf(sud, sud.length);
	...
	public static void usage() {
		System.out.println("Backtracking2D puzzle1.sud [puzzle2.sud ...]");
